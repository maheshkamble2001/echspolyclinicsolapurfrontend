...
export function StatusCell({ getValue, row, column, table, disabled=false}) {
  ...
  const onChange = async (checked) => {
    setLoading(true);
    try {
          const id = row.original.id|| row.original.doctorId||row.original.schemeId ||row.original.GalleryId ||row.original.ID||row.original.customerId|| row.original.user_id || row.original.usertypeid || row.original.BannerId  || row.original.PageID || row.original.categoryId;
